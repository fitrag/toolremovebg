        ...
        <div class="flex items-center justify-center px-10 py-10 sm:py-10 md:py-10 lg:py-10 xl:py-20 bg-gradient-to-r from-violet-500 to-fuchsia-500 text-white">
            <div class="w-[50%] hidden md:hidden lg:hiddenn xl:block">
                <div class="flex flex-col items-center">
                    <img src="/hero.png" alt="" class="w-[425px]">
                </div>
                <p class="text-2xl">
                    Solusi cepat dan mudah untuk menghapus latar belakang foto Anda. Unggah gambar Anda dan dapatkan hasil yang bersih dan profesional dalam hitungan detik, tanpa perlu keahlian desain. Cocok untuk keperluan pribadi, bisnis, dan e-commerce.
                </p>
            </div>
            <div class="flex-auto">
                <div class="flex justfy-center items-center flex-col xl:my-10 lg:my-2 px-[40px]">
                    <div class="sm:flex md:flex lg:flex flex-col items-center justify-center">
                        ...
                        <div v-if="isFilled == true && isFinish != true">
                            <div class="flex flex-col bg-white w-[600px] p-10 rounded-xl items-center text-black shadow-md">
                                <div class="relative border-dashed border-2 border-purple-400 p-10 text-center items-center flex flex-col text-center w-full h-full rounded-xl">
                                    ...
                                    <div v-if="isLoading" class="bg-white opacity-75 absolute top-0 left-0 right-0 bottom-0 block flex h-100 items-center justify-center">
                        ...
                        <div v-if="isFinish">
                            <div class="flex flex-col bg-white w-[600px] p-10 rounded-xl items-center text-black shadow-md">
                                <div style="background:url('https://thumbs.dreamstime.com/b/transparent-seamless-pattern-background-checkered-simulation-alpha-channel-png-wallpaper-empty-gird-grid-vector-illustration-308566526.jpg');z-index:0" class="relative border-dashed border-2 border-purple-400 p-10 text-center items-center flex flex-col text-center w-full h-full rounded-xl">
                                    <button class="bg-slate-100 absolute right-[-10px] top-[-10px] py-0 px-2 text-3xl font-bold rounded-full hover:bg-slate-300" @click="removeImage" style="z-index: 99;">&times;</button>
                                    <img :src="data.hasil" alt="" class="w-100">
                                </div>
                                <a :href="data.hasil" class="bg-purple-400 text-white py-3 text-center w-full block mt-2 rounded font-semibold" v-bind:download="this.file.name.split('.')[0]+`-hapuslatar`+this.file.name.split('.')[1]">Unduh Hasil</a>
                            </div>
                        </div>
                        <div v-if="!isFilled">
                            <label for="file" name="file" class="flex flex-col bg-white w-[600px] p-10 rounded-xl items-center text-black shadow-md">
  ...
  import axios from 'axios'
    ...
    methods: {
        ...
        submitFile(){
            ...
            axios.post('http://127.0.0.1:3000/remove',
